...
static char **_getFilesFromDir(const char *dir, const char **fmts,
    const bool recurse);
...
char **getFiles(const Arguments *args) {
    ...
    for (int i = 0; args->inPaths[i]; i++) {
        ...
        if (files) {
            while (files[fileCount]) fileCount += 1;
        }

        printf("%s (found %s%zu%s files)%s\n\n", COLOR_DEFAULT, COLOR_ACCENT,
            fileCount, COLOR_DEFAULT, COLOR_DEFAULT);
