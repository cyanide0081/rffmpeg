...
static void displayHelp(void) {
    ...
    printf_s("\t%-25sstore converted files in a new folder - you can choose a folder name with '--newfolder=name' (without whitespaces)\n", OPT_MAKENEWFOLDER_STRING);
    printf_s("\t%-25sauto-overwrite existing output files (default behaviour appends an index)\n", OPT_FORCEOVERWRITE_STRING);
    printf_s("\t%-25sdelete original files after conversion \n", OPT_DELETEOLDFILES_STRING);
